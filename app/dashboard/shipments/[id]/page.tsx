...
export default function ShipmentDetailPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    const fetchShipmentDetails = async () => {
      try {
        ...
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/GetShipment?shipment_id=${shipmentId}`, {
          headers: {
  ...
  const formatPostcode = (postcode: number) => {
    return postcode === 0 ? "Not specified" : postcode.toString()
  }
    ...
    return (
      <div className="p-6">
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
          <p className="text-gray-700">No shipment found with ID: {shipmentId}</p>
        </div>
      </div>
    )
  }
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="flex items-start">
              ...
              <div>
                <p className="text-sm text-gray-500">Pickup Postcode</p>
                <p className="font-medium">{formatPostcode(shipment.pick_up_address)}</p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              <MapPin className="h-5 w-5 text-gray-400 mr-3 mt-0.5" />
              <div>
                <p className="text-sm text-gray-500">Delivery Postcode</p>
                <p className="font-medium">{formatPostcode(shipment.delivery_address)}</p>
              </div>
            </div>
          </div>
        </div>
